...
params:
  ...
  studypath: "../../../ds004230/"
  derivatives_dir: "../../../ds004230/derivatives/"
...
# Fixed seed. Model fitting here draws random starting values, so without a
# seed a re-run of the same configuration gives slightly different numbers.
set.seed(123)

configpath <- params$configpath
configname <- basename(configpath)

# Fills in the fields this config does not carry, and translates the names of
# those which have been renamed, so that everything below reads one set of
# names and defaults.
config <- resolve_config(jsonlite::fromJSON(configpath))
...
# An exclusion-only configuration ("-H01" in a field) contributes no columns to
# the inclusion grid, so counting columns alone would read it as "no subsetting
# requested" and silently skip it.
subsetting <- (ncol(config_subset) > 0) ||
  length(attr(config_subset, "exclusions") %||% list()) > 0
no_subsetting <- !subsetting
```

```{r load-data}
#| eval: !expr no_subsetting
#| echo: !expr no_subsetting
...
```{r subsetting}
#| eval: !expr subsetting
#| echo: !expr subsetting
# Subsetting

### Extract measurement information
measurements <- bids_parse_filenames(params$studypath) %>%
  mutate(petinfo = map(filedata,
                            kinfitr:::bids_parse_petinfo))

has_blood <- function(df) {
  any(df$measurement %in% c("blood"))
}

measurements <- measurements %>%
  filter(map_lgl(filedata, has_blood))
...
### Filter
# Keeps what the Subsets block includes and drops what it excludes. An
# attribute the dataset does not carry is reported here rather than failing
# deep inside the join.
measurements <- apply_config_subsets(measurements, config_subset)
...
Now we remove measurements which are missing blood

```{r}
bidsdata <- bidsdata %>% 
    filter(map_lgl(blooddata, ~class(.x) == "blooddata"))
```

```{r}
#| echo: false

merge_runs_config <- config$MergeRuns
```

```{r merge-runs}
#| echo: !expr merge_runs_config

# Runs of one injection -- an early and a late block of the same scan --
# describe one blood curve between them, so their samples are combined and
# fitted together. A measurement with a single run is left as it is, run
# entity and all.
if( merge_runs_config ) {

  bidsdata <- merge_runs(bidsdata)

  # The titles named the runs, which a merged measurement no longer has.
  bidsdata$pet <- attributes_to_title(bidsdata)
  bidsdata$fulltitle <- attributes_to_title(bidsdata, all_attributes = T)
...
merged_any <- any(!is.na(bidsdata$merged_runs))
```

```{r}
#| eval: !expr merged_any
#| echo: false
#| results: asis

str_glue("The following measurements were made up of several runs, and have ",
         "been combined. Each run's sample times were placed on the first ",
         "run's time zero, and the combined data are modelled together and ",
         "written out as one set of derivatives, without a run entity in the ",
         "filename.")
```
...
bidsdata %>%
  filter(!is.na(merged_runs)) %>%
  transmute(Measurement = pet, `Runs combined` = merged_runs) %>%
  kable()
```
...
```{r}
#| results: asis
pf_warnings <- pmap_chr(list(bidsdata$blooddata, bidsdata$petinfo, bidsdata$pet), 
                        qc_parentFraction)
...
```{r}
#| results: asis
bpr_warnings <- pmap_chr(list(bidsdata$blooddata, bidsdata$petinfo, bidsdata$pet), 
                         qc_bpr)
...
```{r}
#| results: asis
wb_warnings <- pmap_chr(list(bidsdata$blooddata, bidsdata$petinfo, bidsdata$pet), 
                        qc_wb)
...
```{r pf-modelcompare}
#| eval: !expr choose_best_model
#| echo: !expr choose_best_model
#| fig-height: 4
#| fig-width: 7
...
```{r pf-sigmoid}
#| echo: !expr do_sigmoid
#| eval: !expr do_sigmoid
#| fig-height: 4
#| fig-width: 5
...
```{r pf-invgamma}
#| echo: !expr do_invgamma
#| eval: !expr do_invgamma
#| fig-height: 4
#| fig-width: 5
...
pf_data_long <- pf_data %>% 
  ...
  filter(time > 0) %>% 
  mutate(parentFraction = ifelse(
    parentFraction > 0.999,
    yes=0.999, no = parentFraction
  )) %>% 
  mutate(parentFraction = ifelse(
    parentFraction < 0.001,
    yes=0.001, no = parentFraction
  ))
...
```{r bpr-constant}
#| echo: !expr do_constant
#| eval: !expr do_constant
#| fig-height: 4
#| fig-width: 5
...
```{r bpr-update-fitted}
#| echo: !expr use_fitted
#| eval: !expr use_fitted
...
```{r aif-weighting-setup}
#| echo: false

# Define variables for AIF weighting display
fit_aif <- config$Model$AIF$Method != "Interpolation"

# Get weighting parameters with defaults for display
weightscheme_display <- ifelse(is.null(config$Model$AIF$weightscheme), 2, config$Model$AIF$weightscheme)
method_weights_display <- ifelse(is.null(config$Model$AIF$Method_weights), TRUE, config$Model$AIF$Method_weights)
taper_weights_display <- ifelse(is.null(config$Model$AIF$taper_weights), TRUE, config$Model$AIF$taper_weights)

weight_scheme_text <- ifelse(weightscheme_display == 1, "Uniform weighting", "Time/activity weighting (Method 2)")
```

```{r aif-weighting-info}
#| echo: !expr fit_aif
#| eval: !expr fit_aif
#| results: asis

cat("**Weighting Configuration:** ", weight_scheme_text, " with Method weights: ", method_weights_display, " and Taper weights: ", taper_weights_display, ".\n\n")
```

```{r aif-setup}
#| echo: false

do_triexp <-(config$Model$AIF$Method=="Fit Individually: Triexponential Decay")
...
exclude_early_manual_aif <- !is.null(config$Model$AIF$exclude_manual_during_continuous) &&
                            config$Model$AIF$exclude_manual_during_continuous

# How the triexponential model describes the rise up to the peak. With
# "interp", the rise is linear interpolation through the measured samples and
# only the decay is fitted; with "linear" it is a straight line from t0 to the
# peak, fitted along with the decay.
aif_rise <- config$Model$AIF$rise %||% "interp"
interp_rise <- do_triexp && aif_rise == "interp"
```
...
if( interp_rise ) {
  str_glue("The rise up to the peak is described by linear interpolation ",
           "through the measured samples, and only the decay after the peak ",
           "is fitted. This is more robust, but less flexible, than fitting ",
           "the rise as a straight line.")
} else {
  str_glue("The rise up to the peak is fitted as a straight line from t0 to ",
           "the peak, along with the decay.")
}
```

```{r aif-exclude-manual}
#| echo: !expr exclude_early_manual_aif
#| eval: !expr exclude_early_manual_aif

# Filter out manual samples collected during continuous sampling for AIF
bidsdata <- bidsdata %>%
  mutate(blooddata = map(blooddata, function(bd) {
    aif_data <- bd_extract(bd, output="AIF")
    
    # Find the latest continuous sampling time point
    continuous_data <- aif_data %>% filter(Method == "Continuous")
    if (nrow(continuous_data) > 0) {
      max_continuous_time <- max(continuous_data$time, na.rm = TRUE)
      
      # Filter out discrete samples before the last continuous sample
      aif_data_filtered <- aif_data %>%
        filter(!(Method == "Discrete" & time <= max_continuous_time))
      
      # Update the blooddata object with filtered AIF
      bd <- bd_addfitted(bd, time = aif_data_filtered$time, predicted = aif_data_filtered$aif, modeltype = "AIF")
    }
    return(bd)
  }))
```

Below we save the raw arterial input function values — computed from the modelled parent fraction and blood-to-plasma ratio, but **before** any AIF curve fitting.
...
aif_raw_data <- bidsdata %>%
  ...
  mutate(aif_raw = map(aif_raw, ~.x %>%
    ...
    select(time, recording, whole_blood_radioactivity, plasma_radioactivity,
           blood_plasma_ratio, metabolite_parent_fraction, AIF)
  )) %>%
  select(pet, aif_raw)
...
aif_raw_filenames <- bidsdata %>%
  ...
  mutate(
    ...
    # A merged measurement's curve belongs to all of its runs, and not to the
    # one whose filename it happened to be derived from.
    output_basename = if_else(is.na(merged_runs), output_basename,
                              bids_drop_entity(output_basename, "run")),
...
aif_data <- bidsdata %>%
  ...
  mutate(aif = map(aif, ~{
    # Filter out manual samples during continuous sampling if enabled
    if (exclude_early_manual_aif) {
      continuous_data <- .x %>% filter(Method == "Continuous")
      if (nrow(continuous_data) > 0) {
        max_continuous_time <- max(continuous_data$time, na.rm = TRUE)
        .x <- .x %>% filter(!(Method == "Discrete" & time <= max_continuous_time))
      }
    }
...
if( length(expdecay_props) != 2 || anyNA(expdecay_props) ) {
  # Not named in the config, so this model's own default is used. Read from
  # the model rather than written out here, since it is not the same for
  # every model.
  expdecay_props <- eval(formals(blmod_exp)$expdecay_props)
}

# Set weighting parameters with defaults
weightscheme <- ifelse(is.null(config$Model$AIF$weightscheme), 2, config$Model$AIF$weightscheme)
Method_weights <- ifelse(is.null(config$Model$AIF$Method_weights), TRUE, config$Model$AIF$Method_weights)
taper_weights <- ifelse(is.null(config$Model$AIF$taper_weights), TRUE, config$Model$AIF$taper_weights)

aif_data <- aif_data %>%
  mutate(aif_fit = map(aif, ~blmod_exp(time = .x$time, activity = .x$aif,
                                       Method = .x$Method, 
                                       rise = aif_rise,
                                       expdecay_props = expdecay_props,
...
if( length(expdecay_props) != 2 || anyNA(expdecay_props) ) {
  ...
  # the model rather than written out here, since it is not the same for
  # every model.
  expdecay_props <- eval(formals(blmod_feng)$expdecay_props)
}
...
```{r aif-fengconv}
#| echo: !expr do_fengconv
#| eval: !expr do_fengconv
#| fig-height: 4
#| fig-width: 9
...
if( length(expdecay_props) != 2 || anyNA(expdecay_props) ) {
  ...
  # the model rather than written out here, since it is not the same for
  # every model.
  expdecay_props <- eval(formals(blmod_fengconv)$expdecay_props)
}
...
```{r}
#| eval: !expr parametric_aif
#| echo: false
#| results: asis

if( interp_rise ) {
  str_glue("The fitted parameters are shown below. With an interpolated rise, ",
           "they describe the decay of the curve, and the straight-line ",
           "approximation of its rise: the rise itself is the measured data, ",
           "and the fitted curve is in the interpolated output.")
} else {
  str_glue("The fitted parameters are shown below.")
}
```
...
```{r aif-splines}
#| echo: !expr do_splines
#| eval: !expr do_splines
#| fig-height: 4
#| fig-width: 9
...
kb <- ifelse(config$Model$AIF$spline_kb == "", yes = 10, 
             no = as.numeric(config$Model$AIF$spline_kb))
ka_m <- ifelse(config$Model$AIF$spline_ka_m == "", yes = 10, 
             no = as.numeric(config$Model$AIF$spline_ka_m))
ka_a <- ifelse(config$Model$AIF$spline_ka_a == "", yes = 10, 
             no = as.numeric(config$Model$AIF$spline_ka_a))
...
```{r aif-update_nonpara}
#| echo: !expr nonparam_aif
#| eval: !expr nonparam_aif
bidsdata <- inner_join(bidsdata, aif_data) %>% 
  mutate(preds = map(preds, ~mutate(.x, 
                             aif = as.numeric(
                               ifelse(aif < 0,
...
if( !do_triexp ) {
  config$Model$AIF$rise <- NULL
}

if( !(any(do_triexp, do_feng, do_fengconv)) ) {
  config$Model$AIF$expdecay_props <- NULL
} else {
  # The values actually used, rather than the "NA" which asked for the model's
  # own default
  config$Model$AIF$expdecay_props <- expdecay_props
}
...
# Clean up default weight values (don't save defaults to avoid clutter)
if( !is.null(config$Model$AIF$weightscheme) && config$Model$AIF$weightscheme == 2 ) {
  config$Model$AIF$weightscheme <- NULL
}
if( !is.null(config$Model$AIF$Method_weights) && config$Model$AIF$Method_weights == TRUE ) {
  config$Model$AIF$Method_weights <- NULL  
}
if( !is.null(config$Model$AIF$taper_weights) && config$Model$AIF$taper_weights == TRUE ) {
  config$Model$AIF$taper_weights <- NULL
}
```
...
exclude_early_manual_wb <- !is.null(config$Model$WholeBlood$exclude_manual_during_continuous) && 
                           config$Model$WholeBlood$exclude_manual_during_continuous
```

```{r wb-exclude-manual}
#| echo: !expr exclude_early_manual_wb
#| eval: !expr exclude_early_manual_wb

# Filter out manual samples collected during continuous sampling for Whole Blood
bidsdata <- bidsdata %>%
  mutate(blooddata = map(blooddata, function(bd) {
    wb_data <- bd_extract(bd, output="Blood")
    
    # Find the latest continuous sampling time point
    continuous_data <- wb_data %>% filter(Method == "Continuous")
    if (nrow(continuous_data) > 0) {
      max_continuous_time <- max(continuous_data$time, na.rm = TRUE)
      ...
      # Update the blooddata object with filtered whole blood
      bd <- bd_addfitted(bd, time = wb_data_filtered$time, predicted = wb_data_filtered$activity, modeltype = "Blood")
    }
    return(bd)
  }))
```
...
kb <- ifelse(config$Model$WholeBlood$spline_kb == "", yes = 10, 
             no = as.numeric(config$Model$WholeBlood$spline_kb))
ka_m <- ifelse(config$Model$WholeBlood$spline_ka_m == "", yes = 10, 
             no = as.numeric(config$Model$WholeBlood$spline_ka_m))
ka_a <- ifelse(config$Model$WholeBlood$spline_ka_a == "", yes = 10, 
             no = as.numeric(config$Model$WholeBlood$spline_ka_a))
...
bidsdata <- bidsdata %>% 
  ...
  mutate(output_basename = basename(bloodfilename),
         # A merged measurement's curve belongs to all of its runs, and not to
         # the one whose filename it happened to be derived from.
         output_basename = if_else(is.na(merged_runs), output_basename,
                                   bids_drop_entity(output_basename, "run")),
         output_basename = str_replace(output_basename,
                                       "_recording-manual_blood.json",
                                       "_inputfunction.tsv"),
         output_folder = dirname(bloodfilename),
         output_filename = paste0(params$derivatives_dir, "/bloodstream/", 
                                  params$analysis_foldername, "/",
                                  output_folder, "/", output_basename)) %>% 
  # Blood json file
...
jsonlite::write_json(dataset_description, 
                     path = paste0(params$derivatives_dir, 
                                   "/bloodstream/", params$analysis_foldername, 
...
```{r methods-boilerplate}
#| results: asis

# Build selected_models list for components that used model selection
selected_models <- list()

if (exists("pf_modelcompare")) {
  selected_models$ParentFraction <- pf_modelcompare$scores$Model[1]
}

# Generate methods text with selected model information
methods_text <- generate_methods_boilerplate(
  config,
  selected_models = selected_models,
  merged_runs = sum(!is.na(bidsdata$merged_runs)))
str_glue(methods_text)
```
