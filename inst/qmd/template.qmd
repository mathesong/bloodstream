...
params:
  configpath: "../extdata/config_2022-09-20_id-8kkzd.json"
  studypath: "../../../ds004230/"
format:
  html:
    code-fold: true
...
config$Model$ParentFraction$gam_k <- "6"
```
...
```{r initial-plotting, fig.height=5, fig.width=10}
#| warning: false
#| message: false
#| fig-height: 4
#| fig-width: 8
...
pf_data <- pf_data %>%
  mutate(pf_fit = map(pf, ~mgcv::gam(parentFraction ~ s(time, 
                                       k=pf_gam_k, 
                                       pc=config_pc), 
                                     data=.x, method = "REML")))
...
hgam_preds <- gratia::fitted_values(pf_hgam_fit, data=hgam_preds) %>% 
...
aif_data <- aif_data %>%
  mutate(preds = map(aif_fit, ~tibble(
    time = predtimes) %>%
      mutate(aif = kinfitr:::predict.blood_exp(.x, newdata=list(time=time)))))
...
aif_data <- aif_data %>%
  mutate(preds = map(aif_fit, ~tibble(
    time = predtimes) %>%
      mutate(aif = kinfitr:::predict.blood_feng(.x, newdata=list(time=time)))))

# Plot
pmap(list(aif_data$aif, aif_data$preds, aif_data$pet), plot_aif_preds)
```

```{r aif-fengconv,echo=do_fengconv, eval=do_feng}
#| fig-height: 4
#| fig-width: 9
...
if( is.na(inftime) ) {  # Fit inftime completely
  aif_data <- aif_data %>%
...
aif_data <- aif_data %>%
  mutate(preds = map(aif_fit, ~tibble(
    time = predtimes) %>%
      mutate(aif = kinfitr:::predict.blood_fengconv(.x, newdata=list(time=time)))))
...
aif_data <- aif_data %>%
  mutate(preds = map(aif_fit, ~tibble(
    time = predtimes) %>%
      mutate(aif = kinfitr:::predict.blood_splines(.x, newdata=list(time=time)))))
...
```{r aif-update,echo=fit_aif, eval=fit_aif}
...
wb_data <- wb_data %>%
  mutate(preds = map(wb_fit, ~tibble(
    time = predtimes) %>%
      mutate(activity = kinfitr:::predict.blood_splines(.x, newdata=list(time=time)))))
...
```{r final-plotting, fig.height=5, fig.width=10}
#| warning: false
#| message: false
#| fig-height: 4
#| fig-width: 8
...
bidsdata <- bidsdata %>% 
  ...
  mutate(input = map(input, ~mutate(.x,
         whole_blood_radioactivity = 1000 * whole_blood_radioactivity,
         plasma_radioactivity = 1000 * plasma_radioactivity,
         AIF = 1000 * AIF)))
```

... and then saved to the derivatives folder.
...
walk(bidsdata$cfg_output_filename, 
