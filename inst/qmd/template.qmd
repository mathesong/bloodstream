...
weight_scheme_text <- switch(as.character(weightscheme_display),
                              "1" = "Uniform weighting",
                              "2" = "Time/activity weighting (Method 2)",
                              "3" = "Magnitude weighting |activity| (3)",
                              "4" = "Magnitude weighting sqrt(|activity|) (4)",
                              "5" = "Magnitude weighting activity^2 (5)",
                              paste("Weight scheme", weightscheme_display))
```
...
# Translate monotone checkboxes to basis spline types.
# Use if/else (not ifelse) so a length-N config vector isn't silently truncated.
mono_before  <- if (is.null(config$Model$AIF$spline_monotone_before))  TRUE  else config$Model$AIF$spline_monotone_before
mono_after_m <- if (is.null(config$Model$AIF$spline_monotone_after_m)) FALSE else config$Model$AIF$spline_monotone_after_m
mono_after_a <- if (is.null(config$Model$AIF$spline_monotone_after_a)) FALSE else config$Model$AIF$spline_monotone_after_a
bs_before  <- if (isTRUE(mono_before))  "mpi" else "cr"
bs_after_d <- if (isTRUE(mono_after_m)) "mpd" else "cr"
bs_after_c <- if (isTRUE(mono_after_a)) "mpd" else "cr"
...
aif_data <- aif_data %>%
  mutate(aif_fit = map(aif, ~blmod_splines(time = .x$time, activity = .x$aif,
                                       ...
                                       bs_before = bs_before,
                                       bs_after_d = bs_after_d,
                                       bs_after_c = bs_after_c,
                                       weightscheme = weightscheme,
                                       Method_weights = Method_weights,
                                       taper_weights = taper_weights)))
...
if( !do_splines ) {
  ...
  config$Model$AIF$spline_monotone_before <- NULL
  config$Model$AIF$spline_monotone_after_m <- NULL
  config$Model$AIF$spline_monotone_after_a <- NULL
}
...
# Clean up default monotone values
if( !is.null(config$Model$AIF$spline_monotone_before) && isTRUE(config$Model$AIF$spline_monotone_before) ) {
  config$Model$AIF$spline_monotone_before <- NULL
}
if( !is.null(config$Model$AIF$spline_monotone_after_m) && isFALSE(config$Model$AIF$spline_monotone_after_m) ) {
  config$Model$AIF$spline_monotone_after_m <- NULL
}
if( !is.null(config$Model$AIF$spline_monotone_after_a) && isFALSE(config$Model$AIF$spline_monotone_after_a) ) {
  config$Model$AIF$spline_monotone_after_a <- NULL
}
```
...
```{r wb-weighting-setup}
#| echo: false

# Define variables for WB weighting display
fit_wb <- config$Model$WholeBlood$Method != "Interpolation"

# Get weighting parameters with defaults for display
weightscheme_display   <- if (is.null(config$Model$WholeBlood$weightscheme))   2    else config$Model$WholeBlood$weightscheme
method_weights_display <- if (is.null(config$Model$WholeBlood$Method_weights)) TRUE else config$Model$WholeBlood$Method_weights
taper_weights_display  <- if (is.null(config$Model$WholeBlood$taper_weights))  TRUE else config$Model$WholeBlood$taper_weights
...
# Set weighting parameters with defaults
weightscheme <- ifelse(is.null(config$Model$WholeBlood$weightscheme), 2, config$Model$WholeBlood$weightscheme)
Method_weights <- ifelse(is.null(config$Model$WholeBlood$Method_weights), TRUE, config$Model$WholeBlood$Method_weights)
taper_weights <- ifelse(is.null(config$Model$WholeBlood$taper_weights), TRUE, config$Model$WholeBlood$taper_weights)

# Translate monotone checkboxes to basis spline types.
# Use if/else (not ifelse) so a length-N config vector isn't silently truncated.
mono_before  <- if (is.null(config$Model$WholeBlood$spline_monotone_before))  TRUE  else config$Model$WholeBlood$spline_monotone_before
mono_after_m <- if (is.null(config$Model$WholeBlood$spline_monotone_after_m)) FALSE else config$Model$WholeBlood$spline_monotone_after_m
mono_after_a <- if (is.null(config$Model$WholeBlood$spline_monotone_after_a)) FALSE else config$Model$WholeBlood$spline_monotone_after_a
bs_before  <- if (isTRUE(mono_before))  "mpi" else "cr"
bs_after_d <- if (isTRUE(mono_after_m)) "mpd" else "cr"
bs_after_c <- if (isTRUE(mono_after_a)) "mpd" else "cr"
...
if( !do_splines ) {
  ...
  config$Model$WholeBlood$spline_monotone_before <- NULL
  config$Model$WholeBlood$spline_monotone_after_m <- NULL
  config$Model$WholeBlood$spline_monotone_after_a <- NULL
  # WB weighting fields only apply inside the spline fit, so strip them too
  config$Model$WholeBlood$weightscheme <- NULL
  config$Model$WholeBlood$Method_weights <- NULL
  config$Model$WholeBlood$taper_weights <- NULL
}
...
# Clean up default weight values (don't save defaults to avoid clutter)
if( !is.null(config$Model$WholeBlood$weightscheme) && config$Model$WholeBlood$weightscheme == 2 ) {
  config$Model$WholeBlood$weightscheme <- NULL
}
if( !is.null(config$Model$WholeBlood$Method_weights) && config$Model$WholeBlood$Method_weights == TRUE ) {
  config$Model$WholeBlood$Method_weights <- NULL
}
if( !is.null(config$Model$WholeBlood$taper_weights) && config$Model$WholeBlood$taper_weights == TRUE ) {
  config$Model$WholeBlood$taper_weights <- NULL
}

# Clean up default monotone values
if( !is.null(config$Model$WholeBlood$spline_monotone_before) && isTRUE(config$Model$WholeBlood$spline_monotone_before) ) {
  config$Model$WholeBlood$spline_monotone_before <- NULL
}
if( !is.null(config$Model$WholeBlood$spline_monotone_after_m) && isFALSE(config$Model$WholeBlood$spline_monotone_after_m) ) {
  config$Model$WholeBlood$spline_monotone_after_m <- NULL
}
if( !is.null(config$Model$WholeBlood$spline_monotone_after_a) && isFALSE(config$Model$WholeBlood$spline_monotone_after_a) ) {
  config$Model$WholeBlood$spline_monotone_after_a <- NULL
}
```
...
methods_text <- generate_methods_boilerplate(config, selected_models = selected_models)
