...
### Filter
# The parser no longer fills in entities a filename does not carry, so a
# subset naming an attribute this dataset does not use must say so rather
# than fail inside the join.
missing_subset_attributes <- setdiff(colnames(config_subset),
                                     colnames(measurements))
if (length(missing_subset_attributes) > 0) {
  stop("The config subsets on attributes not present in this dataset: ",
       paste(missing_subset_attributes, collapse = ", "),
